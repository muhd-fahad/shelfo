...
import '../business/tax_provider.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  String get taxLabel => _taxProvider?.taxLabelController.text ?? "Tax";
  ...
  double get _baseSubtotal => _items.fold(0, (sum, item) => sum + item.total);

  double get subtotal {
    if (_taxProvider == null || !_taxProvider!.isTaxEnabled) return _baseSubtotal;
  ...
  double get taxAmount {
    ...
    if (_taxProvider!.pricingMode == TaxPricingMode.inclusive) {
      ...
      return _baseSubtotal - (_baseSubtotal / (1 + (rate / 100)));
    } else {
      // Tax is added on top of _baseSubtotal
      return _baseSubtotal * (rate / 100);
    }
  }
